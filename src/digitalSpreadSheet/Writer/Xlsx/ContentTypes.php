...
class ContentTypes extends WriterPart {
    ...
    public function writeContentTypes(Spreadsheet $spreadsheet, $includeCharts = false) {
        ...
        for ($i = 0; $i < $mediaCount; ++$i) {
            ...
            if ($this->getParentWriter()->getDrawingHashTable()->getByIndex($i) instanceof \phenyxDigitale\digitalSpreadSheet\Worksheet\Drawing) {
                $extension = strtolower($this->getParentWriter()->getDrawingHashTable()->getByIndex($i)->getExtension());
                $mimeType = $this->getImageMimeType($this->getParentWriter()->getDrawingHashTable()->getByIndex($i)->getPath());
            } else if ($this->getParentWriter()->getDrawingHashTable()->getByIndex($i) instanceof MemoryDrawing) {
