...
use phenyxDigitale\digitalSpreadSheet\RichText\Run;
use phenyxDigitale\digitalSpreadSheet\Shared\StringHelper;
use phenyxDigitale\digitalSpreadSheet\Shared\Xls;
...
use phenyxDigitale\digitalSpreadSheet\Style\Protection;
use phenyxDigitale\digitalSpreadSheet\Worksheet\PageSetup;
use phenyxDigitale\digitalSpreadSheet\Worksheet\SheetView;
...
class Worksheet extends BIFFwriter {
     ...
     * Formula parser.
     *
     * @var \phenyxDigitale\digitalSpreadSheet\Writer\Xls\Parser
     */
    private $parser;
     ...
     * Sheet object.
     *
     * @var \phenyxDigitale\digitalSpreadSheet\Worksheet\Worksheet
     */
    public $phpSheet;
     ...
     *
     * @var null|\phenyxDigitale\digitalSpreadSheet\Shared\Escher
     */
    private $escher;
     ...
     * @param \phenyxDigitale\digitalSpreadSheet\Worksheet\Worksheet $phpSheet The worksheet to write
     */
    public function __construct(&$str_total, &$str_unique, &$str_table, &$colors, Parser $parser, $preCalculateFormulas, \phenyxDigitale\digitalSpreadSheet\Worksheet\Worksheet $phpSheet) {
     ...
     *
     * @see \phenyxDigitale\digitalSpreadSheet\Writer\Xls\Workbook::storeWorkbook()
     */
    public function close(): void{
        ...
        if (($defaultWidth = $phpSheet->getDefaultColumnDimension()->getWidth()) < 0) {
            $defaultWidth = \phenyxDigitale\digitalSpreadSheet\Shared\Font::getDefaultColumnWidthByFont($phpSheet->getParentOrThrow()->getDefaultStyle()->getFont());
        }
    ...
    private function writeFormula($row, $col, $formula, $xfIndex, $calculatedValue) {
        ...
        if (isset($calculatedValue)) {
            ...
            } else if (is_int($calculatedValue) || is_float($calculatedValue)) {
                // Numeric value
...
*/
    ...
    private function writeWindow2(): void{
        ...
        // no support in PhenyxXls for selected sheet, therefore sheet is only selected if it is the active sheet
     ...
     * Get Escher object.
     */
    public function getEscher():  ? \phenyxDigitale\digitalSpreadSheet\Shared\Escher {
     ...
     * Set Escher object.
     */
    public function setEscher( ? \phenyxDigitale\digitalSpreadSheet\Shared\Escher $escher) : void{
    ...
    private function writeCFRule(
        ...
        if ($conditional->getConditionType() == Conditional::CONDITION_EXPRESSION) {
            $type = 0x02;
            $operatorType = 0x00;
        } else if ($conditional->getConditionType() == Conditional::CONDITION_CELLIS) {
            $type = 0x01;
        ...
        } else if ($numConditions === 2 && ($conditional->getOperatorType() === Conditional::OPERATOR_BETWEEN)) {
            $conditionalFormulaHelper->processCondition($arrConditions[0], $cellRange);
            $szValue1 = $conditionalFormulaHelper->size();
            $operand1 = $conditionalFormulaHelper->tokens();
        ...
        if ($bFormatFont == 1) {
            ...
            case \phenyxDigitale\digitalSpreadSheet\Style\Font::UNDERLINE_NONE:
                $dataBlockFont .= pack('C', 0x00);
                $fontUnderline = 0;

                break;
            case \phenyxDigitale\digitalSpreadSheet\Style\Font::UNDERLINE_DOUBLE:
                $dataBlockFont .= pack('C', 0x02);
                $fontUnderline = 0;

                break;
            case \phenyxDigitale\digitalSpreadSheet\Style\Font::UNDERLINE_DOUBLEACCOUNTING:
                $dataBlockFont .= pack('C', 0x22);
                $fontUnderline = 0;

                break;
            case \phenyxDigitale\digitalSpreadSheet\Style\Font::UNDERLINE_SINGLE:
                $dataBlockFont .= pack('C', 0x01);
                $fontUnderline = 0;

                break;
            case \phenyxDigitale\digitalSpreadSheet\Style\Font::UNDERLINE_SINGLEACCOUNTING:
