...
use phenyxDigitale\digitalSpreadSheet\Reader\Gnumeric\PageSetup;
use phenyxDigitale\digitalSpreadSheet\Reader\Gnumeric\Properties;
use phenyxDigitale\digitalSpreadSheet\Reader\Gnumeric\Styles;
...
class Gnumeric extends BaseReader {
    ...
    public function listWorksheetNames($filename) {
        ...
        while ($xml->read()) {
            ...
            } else if (self::matchXml($xml, 'Sheets')) {
        ...
        while ($xml->read()) {
            ...
            if (self::matchXml($xml, 'Sheet')) {
                ...
                while ($xml->read()) {
                    ...
                    } else if (self::matchXml($xml, 'MaxCol')) {
                        $xml->read(); //    Move onto the value node
                        $tmpInfo['lastColumnIndex'] = (int) $xml->value;
                        $tmpInfo['totalColumns'] = (int) $xml->value + 1;
                    } else if (self::matchXml($xml, 'MaxRow')) {
                        $xml->read(); //    Move onto the value node
