...
use phenyxDigitale\digitalSpreadSheet\NamedRange;
use phenyxDigitale\digitalSpreadSheet\Reader\Xls\ConditionalFormatting;
use phenyxDigitale\digitalSpreadSheet\Reader\Xls\Style\CellFont;
use phenyxDigitale\digitalSpreadSheet\Reader\Xls\Style\FillPattern;
use phenyxDigitale\digitalSpreadSheet\RichText\RichText;
use phenyxDigitale\digitalSpreadSheet\Shared\CodePage;
use phenyxDigitale\digitalSpreadSheet\Shared\Date;
use phenyxDigitale\digitalSpreadSheet\Shared\Escher;
use phenyxDigitale\digitalSpreadSheet\Shared\Escher\DgContainer\SpgrContainer\SpContainer;
...
use phenyxDigitale\digitalSpreadSheet\Shared\OLE;
use phenyxDigitale\digitalSpreadSheet\Shared\OLERead;
use phenyxDigitale\digitalSpreadSheet\Shared\StringHelper;
use phenyxDigitale\digitalSpreadSheet\Shared\Xls as SharedXls;
...
class Xls extends BaseReader {
    ...
    public function setCodepage(string $codepage): void {
        ...
        if (!CodePage::validate($codepage)) {
            throw new PhenyxXlsException('Unknown codepage: ' . $codepage);
        }
    ...
    /**
     * Reads names of the worksheets from a file, without parsing the whole file to a PhenyxXls object.
     *
    ...
    private function readRecordData($data, $pos, $len) {
        ...
        if ($this->encryption == self::MS_BIFF_CRYPTO_RC4) {
            ...
            $this->rc4Pos = $pos + $len;
        } else if ($this->encryption == self::MS_BIFF_CRYPTO_XOR) {
...
//                                                    ->setAuthor($author)
    ...
    private function readXf(): void{
        ...
        if (!$this->readDataOnly) {
            ...
            } else if (($code = NumberFormat::builtInFormatCode($numberFormatIndex)) !== '') {
            ...
            if ($this->version == self::XLS_BIFF8) {
                ...
                if ($angle <= 90) {
                    ...
                    $rotation = 90 - $angle;
                } else if ($angle == Alignment::TEXTROTATION_STACK_EXCEL) {
    ...
    private function readStyle(): void{
        ...
        if (!$this->readDataOnly) {
            ...
            // user-defined; not supported by PhenyxXls
        }
    ...
    private function readSheet(): void{
        ...
        } else if ($this->version == self::XLS_BIFF7) {
    ...
    private function readExternalBook(): void{
        ...
        } else if (substr($recordData, 2, 2) == pack('CC', 0x01, 0x04)) {
            // internal reference
        ...
        } else if (substr($recordData, 0, 4) == pack('vCC', 0x0001, 0x01, 0x3A)) {
            // add-in function
            // offset: 0; size: 2; 0x0001
            $this->externalBooks[] = [
                'type' => 'addInFunction',
            ];
        } else if (substr($recordData, 0, 2) == pack('v', 0x0000)) {
            // DDE links, OLE links
    ...
    private function readPane(): void{
        ...
        if (!$this->readDataOnly) {
            ...
            // unfrozen panes; split windows; not supported by PhenyxXls core
        }
    ...
    private static function readBIFF8Constant($valueData) {
        ...
        case 0x10: // error code
            ...
            break;
        default:
            throw new PhenyxXlsException('Unsupported BIFF8 constant');
        }
