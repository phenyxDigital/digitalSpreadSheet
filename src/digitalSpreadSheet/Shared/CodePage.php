    ...
    public static function numberToName(int $codePage): string
    {
        if (array_key_exists($codePage, self::$pageArray)) {
            ...
            if (is_array($value)) {
                ...
                throw new PhenyxXlsException("Code page $codePage not implemented on this system.");
            } else {
                return $value;
            }
        }
        if ($codePage == 720 || $codePage == 32769) {
            throw new PhenyxXlsException("Code page $codePage not supported."); //    OEM Arabic
        }
